...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes ( String url ) {
        ...
        try {
            JSONObject jObject = new JSONObject( getEarthquakesAsJson( url ) );
            ...
            for ( int i = 0; i < jFeatures.length(); ++i ) {
                ...
                earthquake.setMagnitude( jProperties.getDouble( "mag" ) );
                earthquake.setLocation( jProperties.getString( "place" ) );
                earthquake.setDate( new Date( jProperties.getLong( "time" ) ) );
                earthquake.setUrl( jProperties.getString( "url" ) );
            ...
            Log.e( "QueryUtils", "Problem getting the earthquake JSON results", e );
        }
        
        return earthquakes;
    }
    
    private static String getEarthquakesAsJson ( String aUrl ) throws IOException {
        HttpURLConnection connection = null;
        InputStream is = null;
        String jsonResponse = "";
        
        try {
            connection = getHttpURLConnection( aUrl );
            
            if ( connection.getResponseCode() == HttpURLConnection.HTTP_OK ) {
                is = connection.getInputStream();
                jsonResponse = getInputStream( is );
            } else {
    ...
    private static HttpURLConnection getHttpURLConnection ( String aUrl ) throws IOException {
    ...
    private static String getInputStream ( InputStream is ) throws IOException {
